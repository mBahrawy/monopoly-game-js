function Game() {
	...
	this.auction = function() {
		...
		<div>
			Highest Bid = $<span id='highestbid'></span> 
			(<span id='highestbidder'></span>)</div>
		<div>
			<span id='currentbidder'></span>, it is your turn to bid.
		</div>
		<div>
			<input class='form-control' id='bid' title='Enter an amount to bid on ${s.name}.'/>
		</div>
		<div>
			<input class='btn btn-success me-2 btn-sm' type='button' value='Bid' onclick='game.auctionBid();' title='Place your bid.' />
			<input class='btn btn-primary me-2 btn-sm' type='button' value='Pass' title='Skip bidding this time.' onclick='game.auctionPass();' />
			<input class='btn btn-danger btn-sm'type='button' value='Exit Auction' title='Stop bidding on ${s.name} altogether.' onclick='if (confirm(\"Are you sure you want to stop bidding on this property altogether?\")) game.auctionExit();' />
		</div>
		`, 'blank');
...
function popup(HTML, action, option) {
	...
	// Ok
		...
		$("#popuptext").append("<div><input class='btn btn-sm btn-success' type='button' value='OK' id='popupclose' /></div>");
		$("#popupclose").focus();
...
function updateDice() {
	...
	if (document.images) {
		...
		element0.src = "assets/images/Die_" + die0 + ".png";
		element0.alt = die0;
		...
		element1.src = "assets/images/Die_" + die1 + ".png";
		element1.alt = die0;
	} else {
		...
		if (sq.owner == turn) {
			...
			if (sq.house >= 1 && sq.house <= 4) {
				for (var x = 1; x <= sq.house; x++) {
					housetext += "<img src='assets/images/house.png' alt='' title='House' class='house' />";
				}
			} else if (sq.hotel) {
				housetext += "<img src='assets/images/hotel.png' alt='' title='Hotel' class='hotel' />";
			}
...
function updateOption() {
	...
	if (checkedproperty < 0 || checkedproperty >= 40) {
		...
		$("#buildings").show();
		document.getElementById("buildings").innerHTML = "<img src='assets/images/house.png' alt='' title='House' class='house' />:&nbsp;" + housesum + "&nbsp;&nbsp;<img src='assets/images/hotel.png' alt='' title='Hotel' class='hotel' />:&nbsp;" + hotelsum;
...
function chanceCommunityChest() {
	...
	if (p.position === 2 || p.position === 17 || p.position === 33) {
		...
		popup("<img src='assets/images/community_chest_icon.png' style='height: 50px; width: 53px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Community Chest:</div><div style='text-align: justify;'>" + communityChestCards[communityChestIndex].text + "</div>", function() {
	...
	// Chance
		...
		popup("<img src='assets/images/chance_icon.png' /><div '>Chance:</div><div>" + chanceCards[chanceIndex].text + "</div>", function() {
	...
	for (var x = 1; x <= pcount; x++) {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (sq.owner == x) {
				...
				if (sq.house == 5) {
					housetext += "<span style='float: right; font-weight: bold;'>1&nbsp;x&nbsp;<img src='assets/images/hotel.png' alt='' title='Hotel' class='hotel' style='float: none;' /></span>";
				} else if (sq.house > 0 && sq.house < 5) {
					housetext += "<span style='float: right; font-weight: bold;'>" + sq.house + "&nbsp;x&nbsp;<img src='assets/images/house.png' alt='' title='House' class='house' style='float: none;' /></span>";
				}
...
function land(increasedRent) {
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			document.getElementById("landed").innerHTML = "<div>You landed on <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();'  class='btn btn-sm ms-2 btn-primary' value='Buy ($" + s.price + ")' title='Buy " + s.name + " for " + s.pricetext + ".'/></div>";
		}
...
function playernumber_onchange() {
	...
	for (var i = 1; i <= pcount; i++) {
		$("#player" + i + "input").css('display','flex');
	}
}
...
window.onload = function() {
	...
	document.getElementById("enlarge0token").innerHTML += '<img src="assets/images/arrow_icon.png" height="40" width="136" alt="" />';
	document.getElementById("enlarge20price").innerHTML += "<img src='assets/images/free_parking_icon.png' height='80' width='72' alt='' style='position: relative; top: -20px;' />";
	document.getElementById("enlarge38token").innerHTML += '<img src="assets/images/tax_icon.png" height="60" width="70" alt="" style="position: relative; top: -20px;" />';
	...
	document.getElementById("enlarge-wrap").innerHTML += "<div id='enlarge40' class='enlarge'><div id='enlarge40color' class='enlarge-color'></div><br /><div id='enlarge40name' class='enlarge-name'>Jail</div><br /><div id='enlarge40price' class='enlarge-price'><img src='assets/images/jake_icon.png' height='80' width='80' alt='' style='position: relative; top: -20px;' /></div><br /><div id='enlarge40token' class='enlarge-token'></div></div>";
	...
	$(".cell-position-holder, #jail").on("mouseover", function(){
		$("#" + this.enlargeId).css('display','flex');

	}).on("mouseout", function() {
		$("#" + this.enlargeId).css('display','none');
